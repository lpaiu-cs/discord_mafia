...
`liar/` 는 `Discord Game Bot` 안에서 Discord 채널 기반 `라이어게임`을 담당한다. 웹 대시보드는 쓰지 않고, 생성된 채널 안에서 버튼, 일반 메시지, `!투표` prefix, `/제시어` ephemeral 응답, `/liar stats` 전적 조회만으로 한 판을 운영한다. 오디오는 방장이 있는 음성 채널에 공용 브로드캐스트로만 송출한다.
...
- `모드B`: 라이어는 자신이 라이어인지 모른 채 다른 카테고리의 제시어를 받는다
- `모드A` 는 로비에서 `모드A` 버튼을 고른 뒤 카테고리를 고르고 시작한다
- `모드B` 는 로비 카테고리를 직접 고르지 않고, 시작 시 시민 카테고리와 라이어 카테고리를 전체 풀에서 자동 배정한다
- `모드B` 진행 중 공개 상태 메시지에는 카테고리를 노출하지 않는다
...
- 상태 저장은 `in-memory`
- 로비 생성 시 봇이 방장의 현재 음성 채널에 자동 접속
- 라이어 오디오는 개인 UI 효과음 없이 공용 BGM/SFX 만 브로드캐스트
...
- 이 모듈은 Discord `Guild Voice States Intent` 가 켜져 있어야 방장 음성 채널 추적과 자동 오디오 연결을 처리할 수 있다.
- 참가자 잡음 메시지를 자동 정리하려면 봇에 `메시지 관리` 권한이 있는 편이 좋다. 권한이 없으면 안내 메시지만 보낸다.
- `/liar create` 를 쓰는 방장은 먼저 음성 채널에 들어가 있어야 한다.
