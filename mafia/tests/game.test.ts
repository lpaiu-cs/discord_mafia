...
test("연인 희생은 비공개로 처리되고 실제 마피아 사망자 기준으로 도굴과 접선이 판정된다", async () => {
  ...
  assert.ok(game.getAudioCuesForUser("graverobber").some((cue) => cue.key === "ghoul"));
  assert.equal(game.getAudioCuesForUser("mafia").some((cue) => cue.key === "ghoul"), false);
});
...
test("의사 치료로 실제 사망이 없으면 도굴과 짐승인간 접선이 발생하지 않는다", async () => {
  ...
  assert.ok(summary.publicLines.includes("시민 님이 의사의 치료를 통해 살아났습니다!"));
  assert.ok(game.getAudioCuesForUser("doctor").some((cue) => cue.key === "doctor_save"));
  assert.ok(game.getAudioCuesForUser("target").some((cue) => cue.key === "doctor_save"));
});
...
test("경찰이 마피아를 적중하면 개인 메모 확정 정보가 저장된다", async () => {
  const game = createTestGame("balance");
  seedPlayers(game, [
    { userId: "police", displayName: "경찰", role: "police" },
    { userId: "mafia", displayName: "마피아", role: "mafia" },
  ]);
  game.phase = "night";
  setNightAction(game, "police", "policeInspect", "mafia", 1);

  await resolveNight(game);

  assert.deepEqual(game.getConfirmedRoleForViewer("police", "mafia"), {
    role: "mafia",
    source: "police",
  });
});

test("유혹 오디오 큐는 실제 대상에게만 전달된다", async () => {
  const game = createTestGame("balance");
  seedPlayers(game, [
    { userId: "madam", displayName: "마담", role: "madam" },
    { userId: "target", displayName: "대상", role: "citizen" },
    { userId: "other", displayName: "다른이", role: "citizen" },
  ]);
  game.phase = "vote";
  game.phaseContext = phaseContext(12);
  ...
  assert.deepEqual(
    game.getAudioCuesForUser("target").map((cue) => cue.key),
    ["charm"],
  );
  assert.deepEqual(game.getAudioCuesForUser("other"), []);
});
...
test("성직자는 이번 밤 사망자만 후속 대상으로 받고 그 대상을 부활시킨다", async () => {
  ...
  assert.ok(game.getAudioCuesForUser("mafia").some((cue) => cue.key === "gunshots"));
  assert.ok(game.getAudioCuesForUser("priest").some((cue) => cue.key === "revive"));
  assert.ok(game.getAudioCuesForUser("target").some((cue) => cue.key === "revive"));
});
...
test("낮 투표는 죽은 플레이어를 대상으로 받을 수 없다", async () => {
  const game = createTestGame("balance");
  seedPlayers(game, [
    { userId: "voter", displayName: "유권자", role: "citizen" },
    { userId: "dead", displayName: "망자", role: "mafia", alive: false },
  ]);
  game.phase = "vote";
  game.phaseContext = phaseContext(8);
  game["sendOrUpdateStatus"] = async () => undefined;

  await assert.rejects(
    () => game.submitVote({} as Client, "voter", "dead", 8),
    /살아 있는 투표 대상만 선택할 수 있습니다/,
  );
  assert.equal(game.dayVotes.has("voter"), false);
});
...
test("토론 시간 조절은 누른 사람 이름과 함께 공개 채팅 로그를 남긴다", async () => {
  ...
  assert.equal(result, "토론 시간을 10초 늘렸습니다.");
  assert.ok(game.webChats.public.some((message) => message.kind === "system" && message.content === "민재 님이 토론 시간을 10초 늘렸습니다."));
});
...
test("건달 협박 대상은 공개 투표 안내와 상태 embed 에 노출되지 않는다", async () => {
  const game = createTestGame("balance");
  seedPlayers(game, [
    { userId: "thug", displayName: "건달", role: "thug" },
    { userId: "target", displayName: "민재", role: "citizen" },
  ]);
  game.phase = "vote";
  game.phaseContext = phaseContext(14);
  game.bulliedToday.add("target");

  let phasePayload: any = null;
  game["sendPhaseMessage"] = async (_client: Client, options: unknown) => {
    phasePayload = options;
  };

  await game.sendVotePrompt({} as Client);

  assert.equal(phasePayload?.extraLines, undefined);
  const aliveField = game.buildStatusEmbed().toJSON().fields?.find((field) => field.name === "생존");
  assert.ok(aliveField);
  assert.doesNotMatch(String(aliveField?.value ?? ""), /협박/);
});

test("밤 결과 공개 로그는 낮 시작 시 공개 채팅에 중복 적재되지 않는다", async () => {
  const game = createTestGame("balance");
  seedPlayers(game, [
    { userId: "mafia", displayName: "루나", role: "mafia" },
    { userId: "target", displayName: "민재", role: "citizen" },
    { userId: "citizen-1", displayName: "서아", role: "citizen" },
    { userId: "citizen-2", displayName: "도윤", role: "citizen" },
  ]);
  game.phase = "night";
  game.nightNumber = 1;
  game.phaseContext = phaseContext(11);
  setNightAction(game, "mafia", "mafiaKill", "target", 1);
  game["safeSendDm"] = async () => undefined;
  game["syncSecretChannels"] = async () => undefined;
  game["sendPhaseMessage"] = async () => undefined;
  game["sendOrUpdateStatus"] = async () => undefined;
  ...
  await game["finishNight"]({} as Client);

  const deathLines = game.webChats.public
    .filter((message) => message.kind === "system")
    .map((message) => message.content)
    .filter((content) => content === "민재 님이 밤사이 사망했습니다.");

  assert.equal(deathLines.length, 1);
});
...
test("낮 시작 공개 로그는 몇 번째 날인지와 밤 결과를 함께 공개 채팅에 남긴다", async () => {
  ...
  assert.ok(game.lastPublicLines.includes("나미 님이 마피아에게 살해당했습니다."));
});
...
test("밤 행동은 서버에서 역할별 허용 action 을 직접 검증한다", async () => {
  const game = createTestGame("balance");
  seedPlayers(game, [
    { userId: "citizen", displayName: "시민", role: "citizen" },
    { userId: "target", displayName: "대상", role: "mafia" },
  ]);
  game.phase = "night";
  game.phaseContext = phaseContext(9);

  await assert.rejects(
    () =>
      game.submitNightSelection(
        {} as Client,
        {
          kind: "night",
          actorId: "citizen",
    ...
    /이번 밤에 사용할 수 있는 능력이 없습니다|현재 직업으로는 그 행동을 사용할 수 없습니다/,
  );
  assert.equal(game.nightActions.has("citizen"), false);
});

test("잘못된 밤 대상은 거부되고 상태를 오염시키지 않는다", async () => {
  const game = createTestGame("balance");
  seedPlayers(game, [
    { userId: "doctor", displayName: "의사", role: "doctor" },
    { userId: "target", displayName: "대상", role: "citizen" },
  ...
  game.phase = "night";
  game.phaseContext = phaseContext(10);
  ...
  assert.equal(game.nightActions.has("doctor"), false);
});
