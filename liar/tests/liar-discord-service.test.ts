...
test("설명 단계에서 비차례 참가자의 메시지는 정리되고 안내된다", async () => {
  ...
  assert.ok(channel.sent.some((payload) => payload.content.includes("설명 차례입니다")));
});
...
test("게임 시작 뒤 참가자가 서버를 나가면 게임이 취소 종료된다", async () => {
  ...
  assert.ok(channel.sent.some((payload) => payload.content.includes("게임 종료")));
});
...
test("/liar create 는 새 로비를 만들고 상태 메시지를 띄운다", async () => {
  ...
  assert.ok(channel.sent.some((payload) => payload.content.includes("라이어게임 로비가 열렸습니다")));
});
