...
test("자유 토론 중 참가자 과반이 !스킵 을 입력하면 즉시 투표 단계로 넘어간다", async () => {
  const { service, game } = createServiceWithGame();
  const channel = createFakeChannel();
  const client = { channels: { fetch: async () => channel } } as any;
  ...
  for (const [index, author] of skipAuthors.entries()) {
    const { message } = createFakeMessage({
      author: { id: author.id, bot: false, username: author.name },
  ...
  assert.ok(channel.sent.some((payload) => typeof payload.content === "string" && payload.content.includes("토론 스킵이 과반")));
});
...
test("취소 종료된 게임은 상태 메시지 갱신 뒤 음성 연결도 정리한다", async () => {
  ...
  game.forceEnd("방장이 게임을 종료했습니다.");
  await (service as any).resetPhaseState(client, game, channel);

  assert.ok(audio.calls.includes("sync:ended"));
